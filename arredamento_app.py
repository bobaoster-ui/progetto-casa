...
st.set_page_config(page_title="Monitoraggio Arredamento V30.5", layout="wide", page_icon="💎")
...
# --- CLASSE PDF FIX (Rimosso .encode problematico) ---
class PDF(FPDF):
    def header(self):
...
if "password_correct" not in st.session_state:
    ...
    # Lettura dati (Cache disattivata per sync immediato)
    ...
    if "Riepilogo" in sel:
        ...
        if not df_real.empty:
            ...
            # --- BUDGET VISIBILE ---
            budget_max = st.sidebar.number_input("Budget Obiettivo (€)", value=50000, step=1000)
            residuo_budget = budget_max - conf
            st.markdown(f"### 📊 Stato Spese: **{conf:,.2f}€** / **{budget_max:,.2f}€**")
            st.progress(min(conf / budget_max, 1.0))
            if residuo_budget < 0:
                st.warning(f"⚠️ Attenzione: Sei fuori budget di {-residuo_budget:,.2f}€")
            else:
                st.info(f"✅ Budget rimanente: {residuo_budget:,.2f}€")
            ...
            if st.button("📑 Scarica Report Riepilogo"):
                pdf = PDF()
                pdf.add_page(); pdf.set_font('Arial', 'B', 12)
                pdf.cell(0, 10, f"Totale Impegnato: {conf:,.2f} EUR", ln=True)
                pdf.cell(0, 10, f"Totale Versato: {pag:,.2f} EUR", ln=True)
                pdf.cell(0, 10, f"Residuo da Pagare: {conf-pag:,.2f} EUR", ln=True)
                st.download_button("Clicca per scaricare PDF", pdf.output(dest='S'), "Report_Jacopo.pdf", "application/pdf")
        ...
        with st.form(f"form_{sn}"):
            check_chiusura = st.checkbox("🔒 Chiudi Stanza (Sigillo Oro)", value=df_s['stanza_chiusa'].any() if not df_s.empty else False)
            s_cfg = {
            ...
            if st.form_submit_button("💾 SALVA"):
                ...
                for _, row in df_e.iterrows():
                    if row['articolo']:
                        p_p = float(row.get('prezzo_pieno', 0) or 0)
                        sc = float(row.get('sconto_percentuale', 0) or 0)
                        c_u = p_p * (1 - (sc/100)) if p_p > 0 else float(row.get('costo', 0) or 0)
                        scad = str(row['scadenza']) if pd.notnull(row.get('scadenza')) else None
                        supabase.table("arredamento").insert({"stanza": sn, "articolo": str(row['articolo']), "acquistato": float(row.get('acquistato', 1) or 1), "prezzo_pieno": p_p, "sconto_percentuale": sc, "costo": c_u, "importo_totale": c_u * float(row.get('acquistato', 1) or 1), "versato": float(row.get('versato', 0) or 0), "nota": str(row.get('nota', '')), "stato_pagamento": str(row.get('stato_pagamento', '')), "scadenza": scad, "stanza_chiusa": check_chiusura}).execute()
                st.balloons(); st.rerun()
