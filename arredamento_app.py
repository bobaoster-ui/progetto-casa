...
st.set_page_config(page_title="Monitoraggio Arredamento V17.2", layout="wide", page_icon="🏠")
...
if "password_correct" not in st.session_state:
    ...
    if "Riepilogo" in selezione:
        ...
        if all_rows:
            ...
            with m1: st.markdown(f'<div class="metric-card"><div class="metric-label">Confermato</div><div class="metric-value">{tot_conf:,.0f} €</div></div>', unsafe_allow_html=True)
            with m2: st.markdown(f'<div class="metric-card"><div class="metric-label">Pagato</div><div class="metric-value">{tot_versato:,.0f} €</div></div>', unsafe_allow_html=True)
            with m3: st.markdown(f'<div class="metric-card"><div class="metric-label">Da Pagare</div><div class="metric-value">{(tot_conf-tot_versato):,.0f} €</div></div>', unsafe_allow_html=True)

            col_dx, col_sx = st.columns([1, 1.5])
            with col_dx:
                st.markdown('<div style="background-color:white; padding:20px; border-radius:12px;">', unsafe_allow_html=True)
                ...
                if st.button("📄 Genera Report PDF"):
                    ...
                    for _, row in df_final.iterrows():
                        ...
                        pdf.cell(35, h, f"{row['Importo Totale']:,.2f}", 1)
    ...
    # --- STANZE ---
        ...
        with st.form(f"f_{stanza_nome}"):
            # --- FIX: REINSERITA DISCESA STATO PAGAMENTO ---
            c_config = {
                col_sn: st.column_config.SelectboxColumn(col_sn, options=["S", "N"]),
                col_stato: st.column_config.SelectboxColumn(col_stato, options=["", "Da Ordinare", "Ordinato", "In Arrivo", "Saldato"]),
