...
st.set_page_config(page_title="Monitoraggio Arredamento V22.9.17", layout="wide", page_icon="🚀")

# --- INIZIALIZZAZIONE SESSIONE ---
if "dark_mode" not in st.session_state: st.session_state.dark_mode = False
if "password_correct" not in st.session_state: st.session_state.password_correct = False

# --- MOTORE PDF (Fix Scalini e Bytearray) ---
class PDF(FPDF):
    def header(self):
    ...
    def draw_table_row(self, data, w):
        ...
        # Calcolo altezza necessaria per la riga (evita scalini)
        line_heights = [len(self.multi_cell(w[i], 8, str(txt), split_only=True)) for i, txt in enumerate(data)]
        h = max(line_heights) * 8
        if self.get_y() + h > 270: self.add_page()

        curr_x, curr_y = self.get_x(), self.get_y()
        for i, txt in enumerate(data):
            self.set_xy(curr_x + sum(w[:i]), curr_y)
            txt_clean = str(txt).encode('latin-1','replace').decode('latin-1')
            self.multi_cell(w[i], h, txt_clean, border=1, align='L' if i < 2 else 'R')
        self.set_y(curr_y + h)
...
if not st.session_state.password_correct:
    ...
    with st.sidebar:
        # LOGO RIPRISTINATO
        st.markdown(f"""
            <div style="text-align: center; background: white; padding: 10px; border-radius: 10px; margin-bottom: 20px;">
                <img src="https://img.icons8.com/clouds/100/000000/home.png" width="70"><br>
                <b style="color: #2e75b6;">Monitoraggio Arredamento</b><br>
                <small>Jacopo</small>
            </div>
