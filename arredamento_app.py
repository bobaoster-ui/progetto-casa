...
# --- 1. IL SIGILLO DI SICUREZZA (RIGOROSAMENTE 18.1) ---
...
st.set_page_config(
    page_title="Monitoraggio Arredamento V18.12",
    layout="wide",
    page_icon="🚀"
)
...
if "password_correct" not in st.session_state:
    ...
    with st.sidebar:
        ...
        selezione = st.selectbox(
            "MENU NAVIGAZIONE",
            ["🏠 Riepilogo Generale", "✨ Wishlist"] + [f"📦 {s.capitalize()}" for s in stanze_reali]
        )
    ...
    if "Riepilogo" in selezione:
        ...
        if all_rows:
            ...
            t_conf = df_final['Importo Totale'].sum()
            t_vers = df_final['Versato'].sum()
            res = budget_totale - t_conf

            m1, m2, m3, m4 = st.columns(4)
            with m1:
                st.markdown(f'<div class="metric-card"><div class="metric-label">Budget</div><div class="metric-value">{budget_totale:,.0f}€</div></div>', unsafe_allow_html=True)
            with m2:
                st.markdown(f'<div class="metric-card"><div class="metric-label">Confermato</div><div class="metric-value">{t_conf:,.0f}€</div></div>', unsafe_allow_html=True)
            with m3:
                st.markdown(f'<div class="metric-card"><div class="metric-label">Pagato</div><div class="metric-value">{t_vers:,.0f}€</div></div>', unsafe_allow_html=True)
            with m4:
                st.markdown(f'<div class="metric-card"><div class="metric-label">Disponibile</div><div class="metric-value">{res:,.0f}€</div></div>', unsafe_allow_html=True)
            ...
            if st.button("📄 Report PDF"):
                ...
                for _, row in df_final.iterrows():
                    nome = str(row.get('Articolo', row.get('Oggetto', 'N/D'))).encode('latin-1', 'replace').decode('latin-1')
                    y = pdf.get_y()
                    pdf.set_xy(40, y)
                    pdf.multi_cell(90, 10, nome, 1)
                    h = max(pdf.get_y() - y, 10)
                    pdf.set_xy(10, y)
    ...
    # --- STANZE ---
        ...
        stanza = selezione.replace("📦 ", "").lower()
        st.title(f"🏠 {stanza.capitalize()}")
        df = safe_clean_df(conn.read(worksheet=stanza, ttl="1m"))
        c_sn = 'Acquista S/N' if 'Acquista S/N' in df.columns else 'S/N'
        c_st = 'Stato Pagamento' if 'Stato Pagamento' in df.columns else 'Stato'

        with st.form(f"f_{stanza}"):
            config = {
            ...
            if st.form_submit_button("💾 SALVA"):
                for i in range(len(df_edit)):
                    try:
                        r = df_edit.iloc[i]
                        p = float(r.get('Prezzo Pieno', 0))
                        s = float(r.get('Sconto %', 0))
                        q = float(r.get('Acquistato', 1))
                        ...
                        if str(r.get(c_st, "")).strip() == "Saldato":
                            df_edit.at[df_edit.index[i], 'Versato'] = costo * q
                    except:
                        continue
                conn.update(worksheet=stanza, data=df_edit)
                st.cache_data.clear()
                st.balloons()
                st.rerun()
    ...
    # --- WISHLIST ---
        ...
        df_ed_w = st.data_editor(
            df_w,
            use_container_width=True,
            hide_index=True,
            column_config=c_wish,
            num_rows="dynamic" if can_edit_structure else "fixed"
        )
