...
# --- 1. IL SIGILLO DI SICUREZZA (Proprietà Riservata) ---
...
# --- 2. CONFIGURAZIONE PAGINA & CSS (Design V3.5 Style) ---
st.set_page_config(page_title="Monitoraggio Arredamento V17.1", layout="wide", page_icon="🏠")
    ...
    if "Riepilogo" in selezione:
        st.markdown(f"""
            <div class="main-header">
                ...
                <p style="margin:0; opacity: 0.8;">Proprietà: Jacopo - Report del {datetime.now().strftime('%d/%m/%Y')}</p>
            </div>
            """, unsafe_allow_html=True)

        try:
            # Uso di cache 1m per evitare API Limit Error
            df_imp = conn.read(worksheet="Impostazioni", ttl="1m")
        ...
        if all_rows:
            ...
            with m1: st.markdown(f'<div class="metric-card"><div class="metric-label">Budget</div><div class="metric-value">{budget_iniziale:,.0f} €</div></div>', unsafe_allow_html=True)
            ...
            with m4:
                c_disp = "#28a745" if rimanente >= 0 else "#dc3545"
                st.markdown(f'<div class="metric-card"><div class="metric-label">Disponibile</div><div class="metric-value" style="color:{c_disp}">{rimanente:,.0f} €</div></div>', unsafe_allow_html=True)
                ...
                if st.button("📄 Export PDF"):
                    try:
                        ...
                        for _, row in df_final.iterrows():
                            ...
                            h = 10; pdf.cell(30, h, str(row['Ambiente']), 1); pdf.cell(90, h, txt, 1)
                            pdf.cell(35, h, f"{row['Importo Totale']:,.2f}", 1); pdf.cell(35, h, f"{row['Versato']:,.2f}", 1, 1)
                        st.download_button("📥 Scarica Report", data=bytes(pdf.output(dest='S')), file_name="Report.pdf")
            ...
            with col_sx:
                ...
                df_v = df_final[['Ambiente', 'Descrizione_Visualizzata', 'Importo Totale', 'Versato', 'Note']].copy()
                df_v.rename(columns={'Descrizione_Visualizzata': 'Oggetto'}, inplace=True)
                st.dataframe(df_v, use_container_width=True, hide_index=True)
                st.markdown('</div>', unsafe_allow_html=True)

    # --- STANZE (Fix Anti-Stress) ---
    elif "📦" in selezione:
        stanza_nome = selezione.replace("📦 ", "").lower()
        st.title(f"🏠 {stanza_nome.capitalize()}")

        try:
            # ttl="1m" per evitare API Error se clicchi troppo veloce
            df = safe_clean_df(conn.read(worksheet=stanza_nome, ttl="1m"))
        except Exception:
            st.warning("🔄 Google sta caricando, un attimo...")
            time.sleep(2)
            st.rerun()
                ...
                with st.spinner("Salvataggio..."):
                    for i in range(len(df_edit)):
                        try:
                            ...
                            if str(df_edit.iloc[i][col_stato]) == "Saldato":
                                df_edit.at[df_edit.index[i], 'Versato'] = costo * q
                        except: continue
                    conn.update(worksheet=stanza_nome, data=df_edit)
                    st.cache_data.clear() # Fondamentale per vedere subito i dati
        ...
        if st.button("Salva Preferiti"):
            conn.update(worksheet="desideri", data=df_ed_w.drop(columns=['Anteprima'], errors='ignore'))
