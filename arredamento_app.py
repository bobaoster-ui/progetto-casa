...
st.set_page_config(page_title="Monitoraggio Arredamento V22.9.3", layout="wide", page_icon="🏆")
...
def clean_df(df):
    ...
    str_cols = ['Note', 'Acquista S/N', 'S/N', 'Stato Pagamento', 'Stato', 'Link Fattura', 'Link', 'Foto', 'Stanza Chiusa']
    for c in str_cols:
...
if "password_correct" not in st.session_state:
    ...
    if "Riepilogo" in sel:
        ...
        try:
            config_df = conn.read(worksheet="Impostazioni", ttl="5m")
            bud = pd.to_numeric(config_df.iloc[0,1], errors='coerce')
        except: bud = 15000.0
        ...
        for s in stanze:
            try:
                ...
                if not d.empty:
                    ...
                    dc = d[d[cs].str.upper().str.strip() == 'S'].copy()
        ...
        if all_d:
            ...
            if not sc.empty:
                ...
                sc['Data'] = sc['Data Scadenza'].dt.strftime('%d/%m/%Y')
                ...
                st.dataframe(sc.sort_values('gg')[['Stanza','DV','Data','Stato']], use_container_width=True, hide_index=True)
        ...
        try:
            ...
            # Lettura dello stato chiusura con gestione errori e quota
            is_closed = False
            if 'Stanza Chiusa' in df.columns:
                # Confronto robusto: toglie spazi, tutto maiuscolo e cerca "TRUE"
                is_closed = df['Stanza Chiusa'].astype(str).str.strip().str.upper().eq("TRUE").any()
            ...
            with st.form(f"form_{sn}"):
                st.info("💡 Modifica i dati e premi 'SALVA TUTTO' per aggiornare Sheets.")
                ...
                df_e = st.data_editor(df[show_cols], use_container_width=True, hide_index=True, num_rows="dynamic" if edit_struct else "fixed", column_config=cfg)
                ...
                if st.form_submit_button("💾 SALVA TUTTO"):
                    ...
                    for i in range(len(df_e)):
                        try:
                            ...
                            costo = p * (1 - (s / 100)) if p > 0 else float(df_e.iloc[i].get('Costo', 0))
                            df_e.at[df_e.index[i], 'Costo'] = costo
                            df_e.at[df_e.index[i], 'Importo Totale'] = costo * q
                            if str(df_e.iloc[i].get(c_st)) == "Saldato":
                                df_e.at[df_e.index[i], 'Versato'] = costo * q
                        except: continue
            ...
            try:
                ...
                idx = df_c[df_c['Stanza'] == sn].index[0]
                c1, c2, c3 = st.columns(3)
                v1 = c1.checkbox("Montaggio OK", value=bool(df_c.at[idx, 'Montaggio']), key=f"v1_{sn}")
                v2 = c2.checkbox("Integrità", value=bool(df_c.at[idx, 'Integrita']), key=f"v2_{sn}")
                v3 = c3.checkbox("Pulizia", value=bool(df_c.at[idx, 'Pulizia']), key=f"v3_{sn}")

                if st.button(f"Aggiorna Checklist {sn}"):
                    df_c.at[idx, 'Montaggio'], df_c.at[idx, 'Integrita'], df_c.at[idx, 'Pulizia'] = v1, v2, v3
                    conn.update(worksheet="collaudi", data=df_c)
                    st.success("Checklist aggiornata!")
                    st.rerun()
            except: st.warning("Foglio collaudi non accessibile al momento.")

        except Exception as e:
            st.error(f"Errore di connessione o quota: {e}")
            st.info("Attendi 60 secondi e riprova, Google ha limitato le richieste.")
        ...
        try:
            ...
            if st.button("Salva Wishlist"):
                conn.update(worksheet="desideri", data=df_ew.fillna(''))
                st.success("Wishlist salvata!"); st.rerun()
        except: st.error("Errore nel caricamento della Wishlist.")
