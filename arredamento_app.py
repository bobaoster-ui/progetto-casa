...
st.set_page_config(page_title="Monitoraggio Arredamento V8.6", layout="wide", page_icon="🏠")

# Palette Colori
COLOR_AZZURRO = (46, 117, 182)

# --- CLASSE PDF (Corretta per evitare errori di encoding) ---
class PDF(FPDF):
    def header(self):
...
def safe_clean_df(df):
    ...
    # Forziamo i nomi colonne basandoci sui tuoi screenshot
    if 'Articolo' in df.columns and 'Oggetto' not in df.columns:
        df['Oggetto'] = df['Articolo']

    # Colonne numeriche: assicuriamoci siano numeri
    cols_float = ['Importo Totale', 'Versato', 'Prezzo Pieno', 'Sconto %', 'Acquistato', 'Costo']
    for c in cols_float:
        if c in df.columns:
...
if "password_correct" not in st.session_state:
    ...
    if selezione == "Riepilogo Generale":
        ...
        for s in stanze_reali:
            try:
                ...
                if not df_s.empty:
                    # Cerchiamo la colonna S/N (dal tuo screen è 'Acquista S/N')
        ...
        if all_rows:
            ...
            if st.button("📄 Report PDF"):
                try:
                    ...
                    # Fix per l'errore bytearray/encoding visto nello screen
    ...
    # --- 2. STANZE ---
        ...
        # Identifichiamo i nomi colonne esatti dal foglio
        ...
        with st.form(f"form_{selezione}"):
            ...
            if st.form_submit_button("💾 SALVA"):
                # Calcoli su una copia per sicurezza
                for i in range(len(df_edit)):
                    try:
                        ...
                        # LOGICA SALDATO (Rigida su colonna identificata)
