...
st.set_page_config(page_title="Monitoraggio Arredamento V20.3", layout="wide", page_icon="🚀")
...
if "password_correct" not in st.session_state:
    ...
    if "Riepilogo" in sel:
            ...
            with col1:
                ...
                if st.button("📄 Genera Report PDF"):
                    ...
                    for _, r in df.iterrows():
                        ...
                        y = pdf.get_y(); pdf.set_xy(40,y); pdf.multi_cell(90, 10, txt, 1); h = max(pdf.get_y()-y, 10)
        ...
        with st.form(f"f_{sn}"):
            # RIPRISTINO CONFIGURAZIONE COLONNE (Menu a discesa)
            config = {
        ...
        # RIPRISTINO CONFIGURAZIONE WISHLIST (Pulsanti con testo)
        w_config = {
        ...
        if st.button("Salva Wishlist"):
            conn.update(worksheet="desideri", data=dfew.fillna('')); st.cache_data.clear(); st.balloons(); st.rerun()
