...
# --- 1. IL SIGILLO DI SICUREZZA (COPIATO DA 18.1) ---
if st.secrets.get("sicurezza", {}).get("sigillo") != "ATTIVATO":
    st.error("⚠️ LICENZA NON TROVATA")
    st.stop()

# --- 2. CONFIGURAZIONE PAGINA & TEMA (COPIATO DA 18.1) ---
if "dark_mode" not in st.session_state:
    st.session_state.dark_mode = False

st.set_page_config(page_title="Monitoraggio Arredamento V18.9", layout="wide", page_icon="🚀")
...
def safe_clean_df(df):
    """Funzione di pulizia integrale della 18.1"""
    if df is None or df.empty: return pd.DataFrame()
    df.columns = [str(c).strip() for c in df.columns]

    # Gestione testi per evitare errori di tipo
...
if "password_correct" not in st.session_state:
    ...
    if "Riepilogo" in selezione:
        ...
        for s in stanze_reali:
            try:
                ...
                if not df_s.empty:
                    ...
                    # SCADENZARIO PASSIVO (SOLO SE COLONNA ESISTE)
                    if 'Data Scadenza' in df_s.columns:
                        for _, row in df_s.iterrows():
                            if row['Data Scadenza'] and str(row.get(col_stato, "")).strip() != "Saldato":
                                try:
                                    ds = pd.to_datetime(str(row['Data Scadenza']).split(' ')[0], dayfirst=True)
                                    if ds <= datetime.now() + timedelta(days=7):
                                        descr = row.get('Articolo', row.get('Oggetto', 'Elemento'))
                                        alert_scadenze.append(f"⏰ {s.capitalize()}: {descr} ({ds.strftime('%d/%m')})")
                                except: pass
            except: continue

        # Visualizza Alert
        if alert_scadenze:
            for msg in alert_scadenze: st.markdown(f'<div class="alert-box">{msg}</div>', unsafe_allow_html=True)
        ...
        if all_rows:
            ...
            with col_dx:
                ...
                if st.button("📄 Report PDF"):
                    ...
                    for _, row in df_final.iterrows():
                        nome_pdf = str(row.get('Articolo', row.get('Oggetto', 'Elemento'))).encode('latin-1', 'replace').decode('latin-1')
                        start_y = pdf.get_y(); pdf.set_xy(40, start_y); pdf.multi_cell(90, 10, nome_pdf, border=1)
                    ...
                    pdf.set_font('Arial', 'B', 10); pdf.set_fill_color(240, 240, 240); pdf.cell(120, 10, 'TOTALI GENERALI', 1, 0, 'R', True)
            ...
            with col_sx:
                cols_view = [c for c in ['Ambiente', 'Articolo', 'Oggetto', 'Importo Totale', 'Versato'] if c in df_final.columns]
    ...
    # --- STANZE ---
        ...
        with st.form(f"f_{stanza_nome}"):
            ...
            # EDIT DIRETTO SENZA DROP
            df_edit = st.data_editor(df, use_container_width=True, hide_index=True, column_config=c_config, num_rows="dynamic" if can_edit_structure else "fixed")

            if st.form_submit_button("💾 SALVA"):
                for i in range(len(df_edit)):
                    try:
                        p, s, q = float(df_edit.iloc[i].get('Prezzo Pieno', 0)), float(df_edit.iloc[i].get('Sconto %', 0)), float(df_edit.iloc[i].get('Acquistato', 1))
    ...
    # --- WISHLIST ---
        ...
        df_ed_w = st.data_editor(df_w, use_container_width=True, hide_index=True, column_config=c_wish, num_rows="dynamic" if can_edit_structure else "fixed")
